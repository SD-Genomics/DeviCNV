...
export codedir=./Code_v1.5.1/  ### A directory that contains code files.

export outdir=./ExampleOutputs_v1.5.1/  ### A name of your project directory.
...
for inSample in `sed -n '2,$p' ${inSampleInfoTxt} |awk '{print $1}'` ; do
  if [[ ${run_calculateReadDepthOfAmp} == true ]] ; then
    ...
    python ${codedir}/python.calculateReadsDepthOfAmp.py ${inSample} ${inBamDir} ${inAmpliconTxt} ${readDepthDir} ${readDepthStatDir} ${dedupOp} ${datType} ${MQList}
    sleep 1
  fi
done
...
if [[ ${run_mergeReadDepthStatistics} == true ]] ; then
    ...
    python ${codedir}/python.mergeReadDepthStatistics.py ${batchTag} ${inSampleInfoTxt} ${readDepthStatDir}
    sleep 1
fi  
if [[ ${run_chrXNormalizeReadsDepth} == true ]] ; then
  echo STEP3_chrXNormalizeReadsDepth
  python ${codedir}/python.chrXNormalizeReadDepth.py ${batchTag} ${inSampleInfoTxt} ${readDepthDir} ${norReadDepthDir} ${PoolList} ${MQList} 
  sleep 1
fi  
if [[ ${run_filterLowQualSample} == true ]] ; then
  echo STEP4_filterLowQualSample
  Rscript ${codedir}/r.filterLowQualSample.r ${batchTag} ${inSampleInfoTxt} ${norReadDepthDir} ${lqSampleDir} ${MQList}
  sleep 1
fi   
if [[ ${run_buildLinearRegression} == true ]] ; then
  echo STEP5_buildLinearRegression      
  python ${codedir}/python.buildLinearRegression.py ${batchTag} ${readDepthStatDir} ${norReadDepthDir} ${lqSampleDir} ${RDRatioDir} ${MQList} ${dupdelList} 
  sleep 1
fi
