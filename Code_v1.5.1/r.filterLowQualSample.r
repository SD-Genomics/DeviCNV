...
for(MQ in MQList){
  ...
  write.table(outPercentile, file=paste0(lqSampleDir,"/",batchTag,".lowQualitySampleTest.",MQ,".txt"), sep="\t", row.names=F, col.names=T,quote=F)
  ...
  ggsave(filename=paste0(lqSampleDir,"/",batchTag,".lowQualitySampleTest.",MQ,".pdf"), plot=last_plot(), width = (sampleLen*3+100), units= "mm")
}