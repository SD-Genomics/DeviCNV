...
def runBootstriping(sampleList, ampRDDic, medCovSampleList, medCovDic, ampliconID, Pool, MQ, dupTh, delTh, hqSampleList):
  ...
  maxThs=residList[numpy.argsort(residList)][int(-1*round(len(residList)*filterOutPercent))]
  parsedSampleList=[]
  filterOutSampleList=[]
  ...
  for i in range(0,len(y_raw)): 
    ...
    if((numpy.median(yPredictList_t[i])<y_raw[i]) and Pvalue<0.5):
        cnvType="dup"
    elif((numpy.median(yPredictList_t[i])>y_raw[i]) and Pvalue<0.5):
        cnvType="del"
    else:
        cnvType="neutral"
    ### mark low quality sample ################
...
if __name__ == '__main__':
    ...
    for MQ in MQList:
      hqSampleFileName_MQ=lqSampleDir+"/"+batchTag+".lowQualitySampleTest."+MQ+".txt"
      hqSampleList=getHqSampleList(hqSampleFileName_MQ)
      RDStaticFile=readCountStatDir+batchTag+".readDepthStatistics.txt"
