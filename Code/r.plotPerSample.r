...
for(MQ in MQList){
 for(dupdelTh in dupdelList){
  ...
  if(AllPlot==TRUE){
    ...
    copynumber<-pvalues-1
    cnvtypes[is.na(cnvtypes)]<-"unKnown"
    copynumber[cnvtypes=="dup"]<-copynumber[cnvtypes=="dup"]*(-1)
    copynumber[copynumber>0.95]<-1
    copynumber[copynumber<(-0.95)]<-(-1)
    copynumber[copynumber>=(-0.95)&copynumber<=(0.95)]<-0
    ...
    ggplot(dat, aes(x=dat$xvalues,y=dat$CNMvalues, shape=dat$poolvalues, color=dat$pvalues, alpha=dat$alphavalues))+
      ...
      labs(x="All Genes", y="log2(Obs.ReadDepth/Exp.ReadDepth)")+theme_bw()+	 
      ggtitle(titleTXT)+
...
if(GenePlot==TRUE){
  ...
  for(i in 0:(runTime-1)){
    ...
    plot.now<-ggplot()+
      scale_x_continuous(breaks=c())+ 
      scale_y_continuous(limits=c(-3,3), breaks=seq(MINV,MAXV,1))+ 
      labs(x="Exons", y="log2(Obs.ReadDepth/Exp.ReadDepth)")+theme_bw()+
      ggtitle(titleTXT)+
      theme(plot.title = element_text(size=20))+
